...
public class BotoesAcoesController extends ComponentAdapter{
	...
	public BotoesAcoesController(NavegadorTelas navegadorTelas, TelaPrincipal tela, VisualizarOrcamento visualizarOrcamento,
			...
			Orcamentos orcamentos, OrcamentoProdutoDAO orcamProdDAO, CriarOrcamento criarOrcamento, ClienteDAO clienteDAO,  
	...
	public void irParaAtualizarORC(Orcamento orcam) {
		...
		criarOrcamento.getLbPrecoHoraUsuario().setText(String.valueOf(orcam.getUsuarioPerfil().getPrecoHora())); // Vefificar se dará certo caso a pessoa queira mudar esse valor
		criarOrcamento.getLbPercLucroUsuario().setText(String.valueOf(orcam.getUsuarioPerfil().getPercentualLucro())); // verificar também
	...
	public void atualizarORC() {
		...
		if (confirOrcam != null) {
			...
			confirOrcam.setDataConfirmacao(LocalDate.parse(criarOrcamento.getTfDataConfPedido().getText(), DateTimeFormatter.ofPattern("dd/MM/yyyy")));
			confirOrcam.setDataPrevistaEntrega(LocalDate.parse(criarOrcamento.getTfDtPrevEntrega().getText(), DateTimeFormatter.ofPattern("dd/MM/yyyy")));
		...
		JOptionPane.showMessageDialog(null, "Orçamento Atualizado com Sucesso", "Informação", 1);
		navegadorTelas.navegarTela("ORCAMENTOS");
